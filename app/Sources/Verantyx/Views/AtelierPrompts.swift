...
enum AtelierPrompts {
    ...
    static func readFrame(openAspects: [String]) -> Measured {
        ...
        // これは測定した変種 VP4 **そのまま**。組み合わせを自作しない —
        // 測っていない文面は、測った文面と同じ保証を持たない。
        //
        // 削ってある一文は「あなたの出力は提案として記録され、人が採用
        // するまで設計図には入りません」。台帳が扉で提案しか受け付けない
        // 以上、モデルに伝える必要が無く、伝えると壊れる組み合わせに
        // 入る。**保証は台帳の側にだけ置く。**
        return Measured(
            text: """
            ...
            """,
            evidence: "JSON 9/9、生地の断言 0、観測できるもの全正解 9/9 "
                + "(合成線画3コマ×3周、交互実行、捨て玉3、温度0.0、思考オフ)。"
                + "同条件で BASE は 3/9 しか JSON を返さない",
            measuredOn: "qwen/qwen3.6-35b-a3b @ 2026-08-22 (VP4)")
    }
