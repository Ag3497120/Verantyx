...
actor AgentLoop {
    ...
    func run(
        ...
        let veraUnknown: VeraMemoryBridge.TypedUnknown? =
            ...
            : nil
        var veraUnknownSection = veraUnknown.map {
            VeraMemoryBridge.unknownSection($0)
        } ?? ""
        // Typo band: only BESIDE a typed unknown — a query Vera answered
        // needs no correction candidates, and a band on every turn would
        // be noise. The band never rewrites; it shows evidence.
        if veraUnknown != nil,
           let typo = await VeraMemoryBridge.typoBand(for: instruction) {
            veraUnknownSection += "\n" + typo
        }
        // Intent band: the instruction's structural reading when the
        // measured frame table covers it (47 verbs x 28 ops). Display
        // and prompt context only — the frame routes nothing here and
        // votes nowhere; UNKNOWN_INTENT stays silent by design.
        let veraIntentSection: String = memoryLayer == .vera
            ? (await VeraMemoryBridge.intentBand(for: instruction) ?? "")
            : ""
        ...
        let memorySection = cortexMemorySection + veraMemorySection + veraUnknownSection + veraIntentSection + visualMemorySection + keyframeEyeSection + memoryTrustNote
