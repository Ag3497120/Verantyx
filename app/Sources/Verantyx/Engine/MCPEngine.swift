...
enum VeraMemoryPaths {
    ...
    /// shipped here.
    ///
    /// Resolved from the bundle first so an installed .app is self-contained,
    /// then relative to the running binary for a build inside the checkout.
    /// Nothing here is a hardcoded developer path: each candidate is checked
    /// for `photoloset/mcp.py` before it is accepted.
    static func resolvePhotolosetRoot() -> URL? {
        let fm = FileManager.default
        var candidates: [URL] = []
        if let res = Bundle.main.resourceURL {
            candidates.append(res)
        }
        // …/photoloset/app/<DerivedData or build>/…/Verantyx.app/Contents/MacOS
        if var dir = Bundle.main.bundleURL.deletingLastPathComponent()
        ...
        for url in candidates {
            if fm.fileExists(atPath: url.appendingPathComponent("photoloset/mcp.py").path) {
                return url
            }
        }
        return nil
    ...
    /// Stdio MCP launch command for photoloset's own server.
    ///
    /// `/usr/bin/env` runs the tokenised command, so a leading `VAR=value`
    /// assignment is the environment, not a stray argument.
    static func photolosetMCPCommand(root: URL) -> String {
        "PYTHONPATH=\"\(root.path)\" python3 -m photoloset.mcp"
    }
...
final class MCPEngine: ObservableObject {
    ...
    private func loadServers() {
        ...
        } else if let root = VeraMemoryPaths.resolvePhotolosetRoot() {
            // No frozen helper — use photoloset's own MCP server, which is
            // what this build ships. Same tool names, same typed refusals;
            // the five that need the coordinate memory answer
            // UNKNOWN_NOT_IN_THIS_BUILD rather than failing.
            let command = VeraMemoryPaths.photolosetMCPCommand(root: root)
            if let existingIndex = servers.firstIndex(where: { $0.name == "vera-memory" }) {
                var row = servers[existingIndex]
                if row.command != command || !row.isEnabled {
                    row.command = command
                    row.isEnabled = true
                    servers[existingIndex] = row
                    Task { await self.connect(server: row) }
                }
            } else {
                let config = MCPServerConfig(name: "vera-memory", transport: .stdio,
                                             command: command, mode: .ai)
                servers.append(config)
                Task { await self.connect(server: config) }
            }
        } else if let existingIndex = servers.firstIndex(where: { $0.name == "vera-memory" }) {
