...
impl MemoryNode {
    ...
    pub fn parse_jcross(raw: &str) -> Option<Self> {
        ...
        let domain_re = regex::Regex::new(r"【領域】\s*([^\r\n]+)").unwrap();
        ...
        let l1_re = regex::Regex::new(r"\[L1_Cache\]\s*([\s\S]*?)(?:\s*\[|==|---)").unwrap();
        if let Some(cap) = l1_re.captures(raw) {
            node.l1_content = cap[1].trim().to_string();
            node.content = node.l1_content.clone();
        }

        let l2_re = regex::Regex::new(r"\[L2_Archive\]\s*([\s\S]*?)\s*===").unwrap();
...
impl SpatialIndex {
    ...
    pub async fn hydrate(&mut self) -> Result<usize> {
        let mut total = 0;
        let root = &self.root;
        let versions = vec!["jcross_v4", "jcross_v6", "jcross_v6_session", "jcross_v6_full", "jcross_v6_bench_local"];
