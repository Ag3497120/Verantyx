...
export async function clickViaPlaywright(opts: {
  ...
  try {
    if (opts.mouseEntropy && opts.mouseEntropy.length > 0) {
      const box = await locator.boundingBox({ timeout }).catch(() => null);
      if (box) {
        const targetX = box.x + box.width / 2;
        const targetY = box.y + box.height / 2;
        const lastPoint = opts.mouseEntropy[opts.mouseEntropy.length - 1];
        if (lastPoint && lastPoint.length >= 2) {
          const dx = targetX - lastPoint[0];
          const dy = targetY - lastPoint[1];
          for (const pt of opts.mouseEntropy) {
            if (pt.length >= 2) {
              await page.mouse.move(pt[0] + dx, pt[1] + dy, { steps: 1 });
...
export async function hoverViaPlaywright(opts: {
  ...
  try {
    if (opts.mouseEntropy && opts.mouseEntropy.length > 0) {
      const box = await locator.boundingBox({ timeout: resolveInteractionTimeoutMs(opts.timeoutMs) }).catch(() => null);
      if (box) {
        const targetX = box.x + box.width / 2;
        const targetY = box.y + box.height / 2;
        const lastPoint = opts.mouseEntropy[opts.mouseEntropy.length - 1];
...
export async function typeViaPlaywright(opts: {
  ...
  try {
    ...
    if (opts.keyboardEntropy && opts.keyboardEntropy.length > 0) {
      await locator.click({ timeout });
      for (let i = 0; i < text.length; i++) {
        await page.keyboard.type(text[i]);
        const delay = opts.keyboardEntropy[i % opts.keyboardEntropy.length];
        await new Promise(r => setTimeout(r, Math.min(Math.max(delay, 5), 500)));
      }
    } else if (opts.slowly) {
      await locator.click({ timeout });
