...
function normalizeBatchAction(value: unknown): BrowserActRequest {
  ...
  switch (kind) {
    case "click": {
      ...
      const mouseEntropy = Array.isArray(raw.mouseEntropy) ? (raw.mouseEntropy as number[][]) : undefined;
      return {
        kind,
        ...(ref ? { ref } : {}),
        ...(selector ? { selector } : {}),
        ...
        ...(mouseEntropy !== undefined ? { mouseEntropy } : {}),
      };
    }
    case "type": {
      const ref = toStringOrEmpty(raw.ref) || undefined;
      ...
      const timeoutMs = toNumber(raw.timeoutMs);
      const keyboardEntropy = Array.isArray(raw.keyboardEntropy) ? (raw.keyboardEntropy as number[]) : undefined;
      const mouseEntropy = Array.isArray(raw.mouseEntropy) ? (raw.mouseEntropy as number[][]) : undefined;
      return {
        kind,
        ...(ref ? { ref } : {}),
        ...
        ...(timeoutMs !== undefined ? { timeoutMs } : {}),
        ...(keyboardEntropy !== undefined ? { keyboardEntropy } : {}),
        ...(mouseEntropy !== undefined ? { mouseEntropy } : {}),
      };
    }
    case "press": {
        ...
        switch (kind) {
          case "click": {
            ...
            const clickRequest: Parameters<typeof pw.clickViaPlaywright>[0] = {
              ...
              targetId: tab.targetId,
              doubleClick,
              mouseEntropy: Array.isArray(body.mouseEntropy) ? (body.mouseEntropy as number[][]) : undefined,
            };
            if (ref) {
          ...
          case "type": {
            ...
            const typeRequest: Parameters<typeof pw.typeViaPlaywright>[0] = {
              ...
              submit,
              slowly,
              keyboardEntropy: Array.isArray(body.keyboardEntropy) ? (body.keyboardEntropy as number[]) : undefined,
              mouseEntropy: Array.isArray(body.mouseEntropy) ? (body.mouseEntropy as number[][]) : undefined,
            };
            if (ref) {
              typeRequest.ref = ref;
