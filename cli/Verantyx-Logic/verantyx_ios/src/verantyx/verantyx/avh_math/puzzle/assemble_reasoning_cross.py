...
mapping_manager = MappingManager("avh_math/db/text_reasoning_mapping_kb.jsonl")
crystallizer = Crystallizer("avh_math/db/cross_crystals.jsonl")
meta_manager = MetaManager("avh_math/db/meta_crosses.jsonl")

def assemble_reasoning_cross(text: str, cross_db: Any) -> ReasoningCross:
    """
    テキストから分解パズルを作成し、ReasoningCross を構築する。
    """
    # 0. 初期解析 (Lazy import to break circularity)
