...
# Robust imports for solvers
try:
    from avh_math.puzzle.solver_registry import SolverRegistry
    ...
    from avh_math.solvers.citation_solver import CitationSolver
    from avh_math.solvers.symbolic_math import SymbolicMathSolver
except ImportError:
    try:
        from avh_math.puzzle.solver_registry import SolverRegistry
        from avh_math.solvers.prop_solver import PropSolver
        ...
        PropSolver = None
        ModalSolver = None
        ArithmeticSolver = None
        AlgebraSolver = None
        UnitSolver = None
        FormatSolver = None
        LinearAlgebraSolver = None
        CitationSolver = None
        SymbolicMathSolver = None
...
class MathVerifier(BaseVerifier):
    def __init__(self, core_engine: Any = None, kb_path: str = None):
        ...
        self.prop_solver = PropSolver() if PropSolver else None
        self.modal_solver = ModalSolver(max_worlds=3) if ModalSolver else None
        self.arithmetic_solver = ArithmeticSolver() if ArithmeticSolver else None
        self.algebra_solver = AlgebraSolver() if AlgebraSolver else None
        self.unit_solver = UnitSolver() if UnitSolver else None
        self.format_solver = FormatSolver() if FormatSolver else None
        self.linear_algebra_solver = LinearAlgebraSolver() if LinearAlgebraSolver else None
        self.citation_solver = CitationSolver() if CitationSolver else None
        self.symbolic_math_solver = SymbolicMathSolver() if SymbolicMathSolver else None
    ...
    def verify(self, cross: ReasoningCross) -> Dict[str, Any]:
        ...
        # 3. 特殊ドメイン・ソルバーの実行
        # 0. Arithmetic
        if cross.domain == "arithmetic" and self.arithmetic_solver:
        ...
        # Algebra (方程式等)
        if (cross.domain == "algebra" or (target_raw and "==" in target_raw)) and self.algebra_solver:
        ...
        # Format / Unit
        if cross.domain == "format_check" and self.format_solver:
             res = self.format_solver.solve(target_raw)
             result_dict = res.to_dict()
             result_dict["verified_formula"] = target_formula
             return result_dict

        # Linear Algebra (行列、ベクトル)
        if cross.domain == "linear_algebra" and self.linear_algebra_solver:
             res = self.linear_algebra_solver.solve(target_raw)
             if res.status in ("proved", "disproved"):
                 result_dict = res.to_dict()
                 result_dict["verified_formula"] = target_formula
                 return result_dict

        # Symbolic Math (微積分、簡略化)
        if cross.domain == "symbolic_math" and self.symbolic_math_solver:
             res = self.symbolic_math_solver.solve(target_raw)
             if res.status in ("proved", "disproved"):
                 result_dict = res.to_dict()
                 result_dict["verified_formula"] = target_formula
                 return result_dict
        ...
        # Citations
        if cross.domain == "citation" and self.citation_solver:
             res = self.citation_solver.solve(target_raw)
             result_dict = res.to_dict()
             result_dict["verified_formula"] = target_formula
             return result_dict
        ...
        if is_pure_prop and self.prop_solver:
        ...
        # 様相論理 (Kripke 探索)
        if cross.domain in ("modal_logic", "legal_logic", "deontic_logic", "law") and self.modal_solver:
