...
def shape_signature(text: str, core_formula: str | None = None) -> ShapeSig:
    ...
    features: Dict[str, int] = {
        ...
        "has_arithmetic": int(bool(re.search(r"\d", t) and any(op in t for op in ["+", "-", "*", "/", "="]))),
        "has_symbolic_math": int(bool(re.search(r"(diff|int|derive|integral|limit|極限|微分|積分)", t, re.I))),
        "has_citation_words": int(bool(re.search(r"(cite|ref|ソース|出典|文献|according to)", t, re.I))),
    }
    ...
    elif features["has_symbolic_math"]:
        domain_hint = "symbolic_math"
    elif features["has_citation_words"]:
        domain_hint = "citation"
