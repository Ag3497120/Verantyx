    ...
    def build(self, query: str) -> Dict[str, Any]:
        ...
        if not core and not is_set_query:
            ...
            rep = SolutionReport(
                ...
                problem_key=problem_key,
                query=q,
                answer_text="有効な論理式を抽出できなかったため、推論を保留します。"
            )
            rep.trace = trace
            return rep.to_dict()
        ...
        if cross.status == ReasoningStatus.PROVED:
            answer = f"Verified: {display_formula}"
            rep = SolutionReport(status="proved", problem_key=problem_key, query=q, answer_text=answer)
            rep.proof = ProofBlock(method=cross.semantics.get('method', 'logic_engine'), steps=["The proposition was verified as logically valid within the given constraints."])
        elif cross.status == ReasoningStatus.DISPROVED:
            # 決定打：反例が見つかった場合の表示を純化
            answer = f"Counterexample found for: {display_formula}"
            ...
            rep = SolutionReport(status="insufficient_evidence", problem_key=problem_key, query=q, answer_text="Unable to determine validity due to insufficient evidence or ill-formed input.")
            rep.why = cross.metadata.get("reason", "Inconclusive fragments.")
        elif cross.status == ReasoningStatus.SILENT:
             answer = "Insufficient evidence for a definitive answer. Showing preliminary analysis below."
            ...
            # TENTATIVE_ANSWER
            fallback = cross.metadata.get("fallback_answer", {})
            formula = fallback.get("formula", display_formula)
            
            reason = cross.metadata.get("promotion_reason")
            confidence = cross.metadata.get("confidence", 0.0)
            
            if reason == "evidence_exists_post_verification":
                answer = f"Hypothesis based on existing patterns: {formula}"
                note = f"Strict verification was inconclusive, but high similarity to known cases was detected. (Confidence: {confidence:.2f})"
            elif cross.metadata.get("is_repaired"):
                answer = f"Repaired hypothesis: {formula}"
                note = f"The input formula was incomplete; inferred from available structure. (Confidence: {confidence:.2f})"
            else:
                prefix = "[UNVERIFIED] " if cross.metadata.get("low_confidence_warning") else ""
                answer = f"{prefix}Hypothesis based on structural analogy: {formula}"
                note = fallback.get("note", f"Case similarity match. (Confidence: {confidence:.2f})")
