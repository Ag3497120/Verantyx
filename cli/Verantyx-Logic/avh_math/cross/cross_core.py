...
class ReasoningCross:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> 'ReasoningCross':
        ...
        if "verified_formula" in d:
            cross.verified_formula = d["verified_formula"]

        return cross

    def add_collection_item(self, formula: str, status: ReasoningStatus, result_payload: Dict[str, Any]):
        """
        集合検証（SET_ALL/SET_ANY）の各要素をノードとして追加する。
        これが Cross グラフにおける『子のパズルピース』となる。
        """
        item = {
            "node_type": "collection_member",
            "formula": formula,
            "status": status.value,
            "result": result_payload,
            "timestamp": datetime.now().isoformat()
        }
        self.collection_results.append(item)
        
        # 重要な証拠がある場合は evidence にも転記
        if result_payload.get("kb_id"):
            self.evidence.append({
                "kind": "kb_match",
                "id": result_payload["kb_id"],
                "role": "collection_support",
                "target": formula
            })
