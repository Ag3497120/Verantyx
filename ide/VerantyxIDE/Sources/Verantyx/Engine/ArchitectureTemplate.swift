...
struct ArchitectureTemplate: Identifiable, Codable {
    ...
    static let builtins: [ArchitectureTemplate] = [
        ...
        ArchitectureTemplate(
            ...
            description: "Same JGEN for council + execution. Eternal / zone memory as a hidden-state bus. Soft-token steer on L2. Desktop/AX act loop without AgentLoop. No Ollama, no Layer-3 escalation — the path for UI/vision vectors carved into JGEN space.",
            descriptionJA: "合議も実行も同一JGEN。永遠記憶・ゾーン記憶を隠れ状態バスとして使い、L2はソフトトークン誘導＋デスクトップ/AX操作（AgentLoopなし）。Ollama・L3エスカレーションなし — 画面/UIベクトルをJGEN空間に刻むための本線。",
            ...
            layers: [
                ...
                LayerSpec(role: .execution, enabled: true, backend: .jgen, minRAMGB: 6,
                          note: "JGenSpeak / JGenAct (no AgentLoop)", noteJA: "JGenSpeak / JGenAct（AgentLoopなし）"),
