...
#: **この定数も店から読んだもの。** 検査で ``b.seams() == SEAMS`` と
#: 書くと自分自身との比較になる — 縫い目が一本も店に届かなくても
#: 緑のままだった。検査は縫い目の名前をリテラルで持つこと。
SEAMS: List[Dict[str, Any]] = _block.coat().seams()

#: 型紙を3次元に置く初期位置。出所: Block 宣言の ``placement:`` の席。
#: (かつて「settings の腕」と書いてあったが、腕は引き出しでは
#:  なくなった — 腕は主張の種別が決めるもので、置き場所の名前
#:  ではない。ここは ``specific`` なので kind- に載る。)
