...
#: 文字列は Block 宣言の ``formula:`` の席が持ち、ここはそれを出すだけ。
#: (かつて「rules の腕」と書いてあったが、そういう腕は無い —
#:  式は値を**生んだ**ものなので ``derived``、腕は cause+。)
#: **この定数を検査の答え合わせに使ってはいけない。** これは店から
#: 読んだものなので、``b.formulas() == FORMULAS`` は値を自分自身と
#: 比べることになる (実測: 店を丸ごと迂回しても検査は緑のまま)。
